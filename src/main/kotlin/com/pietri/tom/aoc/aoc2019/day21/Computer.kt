...
data class Computer(val instructions: MutableList<BigInteger>) {
    ...
    fun getNexOutput(input: MutableList<Int>): List<BigInteger> {
    ...
    private fun getOutputAndPositionForOpcode(operation: Operation, input: MutableList<Int>): OperationResult {
    ...
    private fun writeBufferValue(input: MutableList<Int>, operation: Operation): Int {
        val param1Addr = getFirstParamAddres(operation.firstParamMode)
        val removeAt = input.removeAt(0)
        setValueAtPosition(param1Addr, BigInteger(removeAt.toString()))
